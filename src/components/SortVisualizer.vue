...
const selectedAlgorithm = ref<SortingAlgorithmOption>({
...
<template>
  <Card
    class="p-4 mx-auto container bg-surface-0 text-surface-900 dark:bg-surface-900 dark:text-surface-0"
  >
    <template #content>
      <div class="grid gap-4">
        <!-- Controls -->
        <div
          class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-md border border-surface-300 dark:border-surface-700 bg-surface-0 dark:bg-surface-800"
        >
          <div>
          ...
          <div>
            ...
            <Select
              ...
              :options="sortingAlgorithmOptions"
              optionLabel="label"
              class="w-full"
            />
          </div>
        ...
        <!-- Step Counter -->
        <div
          class="p-3 rounded-md border border-surface-300 dark:border-surface-700 bg-surface-0 dark:bg-surface-800"
        >
        ...
        <!-- Visualization -->
        <div
          class="relative h-64 p-4 rounded overflow-hidden border border-surface-300 dark:border-surface-700 bg-surface-0 dark:bg-surface-800"
        >
          <TransitionGroup
            :key="generationKey"
            name="flip"
            tag="div"
            class="flex items-end gap-2 h-full"
          >
            <div
              v-for="item in current.items"
              :key="item.id"
              class="relative w-6 flex flex-col items-center justify-end text-xs font-semibold text-surface-0"
            >
              <div
                class="w-full transition-all duration-300 flex items-end justify-center rounded-sm text-surface-0"
                :class="
                  current.highlights.includes(item.id)
